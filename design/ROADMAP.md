...
| Pages | Material 3 GitHub Pages shell, tabbed discovery, repository-backed changelog, command palette, notification centre and responsive documentation surface | **Source/type/lint/build gates pass; 127 focused site tests pass across 13 files, and hosted run 30887123867 passed 4,232 tests, all seven jars, Java render and the Windows installer. Its screenshot job exposed one wizard-tab harness defect; commit `b9be794` follows the visible Make a map tab, with hosted run 30888631389 pending. Runtime/headless capture remains a separate boundary** |
