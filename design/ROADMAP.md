...
| Pages | Material 3 GitHub Pages shell, tabbed discovery, repository-backed changelog, command palette, notification centre and responsive documentation surface | **Source/type/lint/build gates pass locally; the full workspace gate is 300 test files, 4,810 passed, 23 skipped. The current continuation adds every-rendered-element appearance coverage, dynamic per-group discovery searches, searchable tab/group/overflow menus with adjacent builders, and cross-platform config line-ending preservation. Hosted CI/Pages proof remains outstanding; runtime/headless capture remains a separate boundary** |
