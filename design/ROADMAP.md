...
| Pages | Material 3 GitHub Pages shell, tabbed discovery, repository-backed changelog, command palette, notification centre and responsive documentation surface | **Source/type/lint/build gates pass locally; the full workspace gate at the current default tip is 349 test files, 5,602 passed, 3 skipped. The continuation adds every-rendered-element appearance coverage, dynamic per-group discovery searches, searchable tab/group/overflow menus with adjacent builders, and cross-platform config line-ending preservation. GitHub Actions Pages run `30943812059` is green for `80369ec`; the live site returns 200 and its deployed bundle contains menu-search, regex-builder, appearance-coverage and dynamic-group-search markers. The current default tip is `6e90336`; its hosted CI/Pages runs are pending, so a fresh live proof remains required; runtime/headless capture remains a separate boundary** |
