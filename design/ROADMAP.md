...
| Pages | Material 3 GitHub Pages shell, tabbed discovery, repository-backed changelog, command palette, notification centre and responsive documentation surface | **Source/type/lint/build gates pass; 39 focused site tests pass, including complete article commands and one search field plus anchored regex builder per settings tab. Full monorepo evidence remains red: local CRLF HOCON and `@material-bluemap/nbt` failures plus remote world-mount assertion `.minecraft` versus `.` in run 30884892507. Runtime/headless capture is still separate** |
