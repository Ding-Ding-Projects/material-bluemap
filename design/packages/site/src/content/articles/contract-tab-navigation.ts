        ...
        },
        {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    content:
                        "298 tests pass across the 15 files named here: the desktop tab model (tabModel.test.ts, closePlans.test.ts, tabSearch.test.ts, tabStorage.test.ts, tabGroupPicker.test.ts, tabMenus.test.ts, TabbedNavigation.test.ts, tabGroupPickerMount.test.ts, TabResultList.test.ts, all nine under packages/ui/src/components/tabs/) and the Pages site's own (TabStrip.test.ts plus five suites under packages/site/src/search/: engine, evaluator, predicate, query model and tab-matching): overflow, reordering, pinning, grouping, all four searches, both bulk-close directions, appearance-editor discovery through right-click, Shift+right-click and the keyboard path, and persistence restored on the next mount. A wider run of 18 files and 324 tests is also real and is where this page's count used to come from before it drifted: packages/site/src/search/ carries two more suites this page does not name, attachBuilder.test.ts and runSearch.test.ts, and packages/ui/src/copy/surfaces/ carries a second, unrelated tabGroupPicker.test.ts of its own, checking that surface's catalogue coverage rather than its behaviour. A name-based test glob sweeps that file in too; a path-based one does not. This page names the narrower, path-exact 15.",
                },
            ],
        },
    ],
