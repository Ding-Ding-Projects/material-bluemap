...
export const destructiveActionGate: Article = {
    ...
    summary:
        "Two independently operated keys, then a slider that has to travel its whole range, in front of 23 gated destructive actions, with an inventory that fails the build when a new one arrives without one.",
    category: "application",
    status: "shipped",
    statusNote:
        "The gate, both its presentations and the inventory are on the default branch with three test files running in CI, one of which drives both cards through every state. The inventory's KNOWN_GAPS list is empty: signing out of GitHub and the primitive behind it, once declared as gaps, now render their own gate instance and are declared gated. The contract is met on this surface, and this article documents what is built.",
            ...
            blocks: [
                {
                    ...
                    items: [
                        ...
                        [
                            ...
                            "holds a gate, every ungated entry justifying its standing at length, an empty ",
                            "KNOWN_GAPS list, exactly two gate components, both on the shared state ",
                ...
                },
                {
                    kind: "callout",
                    tone: "note",
                    title: "The two gaps this article used to name are closed",
                    content: [
                        "Signing out of GitHub revokes the stored token and, when GitHub honours the revocation, ",
                        "the grant on the account. It now sits behind its own ",
                        { code: "ConfigSuperConfirm" },
                        " instance, closed over the account's id, in ",
                        { code: "GitHubAccountsList.vue" },
                        " and ",
                        { code: "GitHubStatusRow.vue" },
                        ". The inventory's KNOWN_GAPS list, which used to name both the row and the primitive ",
                        "behind it, is empty. Progress was tracked as ",
