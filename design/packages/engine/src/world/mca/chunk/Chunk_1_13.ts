...
import { getByteHalf, getValueFromLongStream, noFloodWarning } from "../MCAUtil.js";
...
const STATUS_FULLCHUNK = new Key("minecraft", "fullchunk");
const STATUS_POSTPROCESSED = new Key("minecraft", "postprocessed");

export const CHUNK_1_13_DATA_TOKEN: TypeToken<Chunk_1_13_Data> = TypeToken.of("Chunk_1_13.Data");
const LEVEL_TOKEN: TypeToken<Level> = TypeToken.of("Chunk_1_13.Level");
const HEIGHTMAPS_TOKEN: TypeToken<HeightmapsData> = TypeToken.of("Chunk_1_13.HeightmapsData");
const SECTION_TOKEN: TypeToken<SectionData> = TypeToken.of("Chunk_1_13.SectionData");

export class Chunk_1_13 extends MCAChunk {
    ...
    constructor(world: MCAWorld, data: Chunk_1_13_Data) {
        ...
        this.worldSurfaceHeightsPresent = this.worldSurfaceHeights.length >= 36;
        this.oceanFloorHeightsPresent = this.oceanFloorHeights.length >= 36;
    ...
    override getBiome(x: number, _y: number, z: number): Biome {
        ...
        const biomeIntIndex = ((z & 0xf) << 4) | (x & 0xf);
    ...
    override getWorldSurfaceY(x: number, z: number): number {
        return getValueFromLongStream(this.worldSurfaceHeights, ((z & 0xf) << 4) | (x & 0xf), 9);
    }
    ...
    override getOceanFloorY(x: number, z: number): number {
        return getValueFromLongStream(this.oceanFloorHeights, ((z & 0xf) << 4) | (x & 0xf), 9);
    }
...
class Section {
    ...
    getBlockState(x: number, y: number, z: number): BlockState {
        ...
        const id = getValueFromLongStream(
            this.blocks,
    ...
    getLightData(x: number, y: number, z: number, target: LightData): LightData {
        ...
        const blockHalfByteIndex = blockByteIndex >> 1;
        const largeHalf = (blockByteIndex & 0x1) !== 0;
...
/** upstream: Chunk_1_13.Data */
export class Chunk_1_13_Data extends MCAChunkData {
...
/** upstream: Chunk_1_13.HeightmapsData */
...
/** upstream: Chunk_1_13.SectionData */
class SectionData {
    y = 0; // @NBTName("Y")
...
const DATA_SCHEMA: ObjectSchema<Chunk_1_13_Data> = {
    create: () => new Chunk_1_13_Data(),
    fields: {
        ...MCA_CHUNK_DATA_FIELDS,
...
export function registerChunk_1_13Schemas(nbt: BlueNBT): void {
    ...
    nbt.register(CHUNK_1_13_DATA_TOKEN, DATA_SCHEMA);
}
