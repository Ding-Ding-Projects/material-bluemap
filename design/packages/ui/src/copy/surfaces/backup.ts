...
export const BACKUP_VOICED = {
    ...
    "backup.whyNotLfs": {
        en: [
            ...
            "This deliberately does not use Git LFS. A free GitHub account gets one gigabyte of LFS storage and one gigabyte of bandwidth a month, and every restore is metered against it, so a single multi-gigabyte world exhausts the free tier and each restore is billed again. Release assets are free on a public repository and capped per file rather than in total. The pointer format is Desktop Material's Cheap LFS v1, so a backup made here is readable there.",
            "This deliberately does not use Git LFS. A free GitHub account gets one gigabyte of LFS storage and one gigabyte of bandwidth a month, and every restore is metered against it, so a single multi-gigabyte world exhausts the free tier on its own and each restore is billed again. Release assets are free on a public repository and capped per file rather than in total. The pointer format is Desktop Material's Cheap LFS v1, so a backup made here is readable there.",
            "This deliberately does not use Git LFS, and the arithmetic is why. A free GitHub account gets one gigabyte of LFS storage and one gigabyte of bandwidth a month, every restore is metered against it, and a single multi-gigabyte world eats the free tier whole and then bills you again on the way back out. Release assets are free on a public repository and capped per file rather than in total. The pointer format is Desktop Material's Cheap LFS v1, so a backup made here is readable there.",
            "This deliberately does not use Git LFS, and the arithmetic is the entire reason. A free GitHub account gets one gigabyte of LFS storage and one gigabyte of bandwidth a month, every restore is metered against it, and a single multi-gigabyte world eats the free tier whole and then charges you again every time you want it back. Release assets are free on a public repository and capped per file rather than in total, which is a far better deal for a world that is mostly chunks. The pointer format is Desktop Material's Cheap LFS v1, so a backup made here is readable there.",
        ],
        yue: [
            "呢度係特登唔用 Git LFS。免費 GitHub 帳戶得 1 GB LFS 儲存空間同每個月 1 GB 頻寬，而且每次還原都會計落去，所以一個幾 GB 大嘅世界就已經食晒個免費額，之後每次還原都要再畀錢。Release asset 喺公開儲存庫係免費嘅，而且係逐個檔案限大細，唔係計總數。個 pointer 格式係 Desktop Material 嘅 Cheap LFS v1，所以喺呢度整嘅備份，喺嗰邊都讀得到。",
            "呢度係特登唔用 Git LFS。免費 GitHub 帳戶得 1 GB LFS 儲存空間同每個月 1 GB 頻寬，而且每次還原都會計落去，所以一個幾 GB 大嘅世界就已經食晒個免費額，之後每次還原都要再畀錢。Release asset 喺公開儲存庫係免費嘅，而且係逐個檔案限大細，唔係計總數。個 pointer 格式係 Desktop Material 嘅 Cheap LFS v1，所以喺呢度整嘅備份，喺嗰邊都讀得到。",
            "呢度係特登唔用 Git LFS。免費 GitHub 帳戶得 1 GB LFS 儲存空間同每個月 1 GB 頻寬，而且每次還原都會計落去，所以淨係一個幾 GB 大嘅世界就已經食晒個免費額，之後每次還原都要再畀錢。Release asset 喺公開儲存庫係免費嘅，而且係逐個檔案限大細，唔係計總數。個 pointer 格式係 Desktop Material 嘅 Cheap LFS v1，所以喺呢度整嘅備份，喺嗰邊都讀得到。",
            "呢度係特登唔用 Git LFS，計條數就明。免費 GitHub 帳戶得 1 GB LFS 儲存空間同每個月 1 GB 頻寬，每次還原都會計落去，一個幾 GB 大嘅世界一啖就食晒個免費額，跟住你想攞返出嚟仲要再畀多次錢。Release asset 喺公開儲存庫係免費嘅，而且係逐個檔案限大細，唔係計總數。個 pointer 格式係 Desktop Material 嘅 Cheap LFS v1，所以喺呢度整嘅備份，喺嗰邊都讀得到。",
            "呢度係特登唔用 Git LFS，成個理由就係計條數。免費 GitHub 帳戶得 1 GB LFS 儲存空間同每個月 1 GB 頻寬，每次還原都會計落去，一個幾 GB 大嘅世界一啖就食晒個免費額，之後你每次想攞返出嚟都要再畀多次錢。Release asset 喺公開儲存庫係免費嘅，而且係逐個檔案限大細，唔係計總數，對住一個成身都係 chunk 嘅世界抵好多。個 pointer 格式係 Desktop Material 嘅 Cheap LFS v1，所以喺呢度整嘅備份，喺嗰邊都讀得到。",
        ],
    },
    /*
