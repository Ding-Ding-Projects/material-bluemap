 ...
 * and, when that runner is remote, a {@link FileTransfer} (rsync-when-available, scp
 * otherwise) to bring bytes back. That is the same seam `runtime/docker.ts` already proved
 * out for reading a remote daemon's *state*; this reuses it for reading a remote daemon's
 * *data*.
 */
...
export class DockerWorldFetcher {
    ...
    async fetch(request: DockerWorldFetchRequest): Promise<DockerWorldFetchResult> {
        const fetchId = fetchIdFor(request.source);
...
function fetchIdFor(source: DockerSourceRequest): string {
