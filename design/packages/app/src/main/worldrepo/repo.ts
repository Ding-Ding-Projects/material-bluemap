 ...
 *   already wrote to, or this computer's own copy having been lost. {@link runSync} closes
 *   it with one addition `pages/hosting.ts` does not need: before resetting the branch, it
 *   fetches the remote branch's objects into the local database (best-effort; a failure
 *   here costs one sync's worth of bandwidth, never correctness) so the exclusion above has
 *   something to exclude against even on a machine that has never synced this target before.
 *
 * ## The marker, again
 *
 ...
 * region file mid-save can be caught torn - {@link preflight} says so. GitHub blocks any
 * single file over 100 MB outright and recommends repositories stay under roughly a
 * gigabyte; both are checked and reported rather than discovered from a rejected push. A
 * push GitHub refuses - a branch protection rule, a size limit, an expired sign-in - is
 * reported with GitHub's own words, never guessed at.
 */
...
import { GIT_COMMAND, normaliseBranch } from "../pages/hosting.js";
