...
function validateDish(dish, expectedId) {
    ...
    const slug = requireString("dish.slug", dish.slug, { min: 1, max: 120, pattern: SAFE_SLUG });
...
function validateAsset(asset, volume, expectedFileName) {
    ...
    const sourceUrl = requireString("asset.browser_download_url", asset.browser_download_url, {
        max: 500,
    });
    let parsed;
    try {
...
function parseArgs(argv) {
    ...
    for (let i = 2; i < argv.length; i++) {
        ...
        if (arg === "--json") args.json = true;
            ...
            if (i + 1 >= argv.length) throw metadataError("arguments.out", "requires a value");
...
async function loadAssetIndex() {
    ...
    const volumes = releases.filter(
        (release) => typeof release?.tag_name === "string" && release.tag_name.startsWith("catalog"),
    );
    if (volumes.length === 0) {
...
async function main() {
    ...
    if (!rawDish) throw new Error(`catalog has no record ${wrappedId} (asked for ${id})`);
    ...
    if (dirname(resolve(outPath)) !== resolve(args.out)) {
        throw metadataError("asset.name", "resolved outside the requested output directory");
    }
