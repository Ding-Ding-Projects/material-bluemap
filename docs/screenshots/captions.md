...
![The maps and servers manager on its own tab, listing the maps rendered on this computer and the remote BlueMap servers the application knows about, with the fields for adding another](profiles-manager.png)
...
![The "Language and tone" settings section, scrolled into view in the settings drawer](settings-section-language-and-tone.png)
...
![The options editor, the "History" tab, with the settings that tab owns](config-tab-history.png)
...
![The notification centre, so a message that has already faded away is still readable, searchable and filterable by level](notifications-history.png)
