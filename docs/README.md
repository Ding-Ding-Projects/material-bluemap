...
| [Drawing a render mask](./render-mask-drawing.md) | The two-way-bound numbers and drawing behind the render mask editor, an honest area/cost estimate, the cloud-render fidelity check that surfaces a mask the cloud path silently ignores, and export/import as a self-describing file. |
...
| [Self-hosted CI dependency bootstrap](./self-hosted-ci-bootstrap.md) | The complete Linux/Windows job inventory, exact check-first dependency profiles, canonical pinned tool downloads, non-interactive failure rules, and the guard that fails when a self-hosted job is added without coverage. |
...
| [A world that lives inside Docker](./docker-world-source.md) | Reaching a world mounted into a container or a named volume — a bind path read directly when it genuinely is one, `docker cp` or a disposable helper container otherwise — the refusal around a server that is still running while its world is read, overridable only by an explicit, per-fetch acknowledgement, never a standing setting — and a cheap change-check fingerprint exposed over IPC the same way the git-repository and SSH routes expose theirs. |
