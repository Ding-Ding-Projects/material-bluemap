...
| [A world that lives inside Docker](./docker-world-source.md) | Reaching a world mounted into a container or a named volume — a bind path read directly when it genuinely is one, `docker cp` or a disposable helper container otherwise — and the one refusal, with no override, around a server that is still running while its world is read. |
